...
simpleStorage.storage.feeds = {"maxi":{ "url":"http://localhost/feed", "poll_interval":9999999999, "entries":{} }};
...
//
//
        Database.get_items("feed", 100,100, function(items){
  ...
  Request.Request({
    ...
    onComplete: function (response) { // and parse it with jQuery using a page worker
      ...
      worker.port.on("transmit-entries", function(entries) {
        ...
        Database.add_items("feed", entries);
      });
