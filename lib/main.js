  ...
  onClick: function() {
    ...
    tab.attach({
      ...
      "onMessage": function(list) {
        ...
        for (var i=0; i<sites.length; i++) {
          var site = sites[i];
          var data = simpleStorage.storage.site_data[site];
...
require("widget").Widget({
  ...
  onClick: function() {
    ...
    for (var i=0; i<sites.length; i++) {
      ...
      var data = simpleStorage.storage.site_data[site];
      for (url in data.identities) { if (!data.identities.hasOwnProperty(url)) continue;
        name = data.identities[url].name;
        avatar = data.identities[url].avatar;
        item.accounts.push({
          url: url,
          avatar: avatar,
          name: name
    ...
    connect_panel.port.emit("list-sites", site_list);
