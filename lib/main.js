...
function deliver() {
  ...
  Database.get_deliver_like(function(likes) { // TODO: give up after a certain number of attempts in a large enough range of time
    ...
    for (var i=0; i<likes.length; i++) {
      ...
      var config = simpleStorage.storage.connections[connection].like_target;
      if (!config) continue;
      ...
      make_request(config, variables, function(response) {
        ...
        Database.register_delivery_attempt(like.id, successful);
      });
    }
  });
}
    ...
    onOpen: function(win) {
      ...
      tab.on("ready", function() {
        ...
        var worker = tab.attach({
          ...
          contentScriptFile: map_urls(config.relogin.files, "sites/"+site),
