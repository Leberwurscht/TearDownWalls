  ...
  worker.port.on("request-posts", function(account, posts, comments, start_date) {
    ...
    Database.get_items(connections, posts, comments, start_date, function(items) {
      ...
      for (id in items) { if (!items.hasOwnProperty(id)) continue;
        var item = items[id];
        var configuration = simpleStorage.storage.connections[item.feed];
...
function create_page_mod(site_identifier, config) {
  ...
  for (var account in configuration) { if (!configuration.hasOwnProperty(account)) continue;
    if (configuration[account].connections.length) crosspost_accounts.push(account);
  }
  ...
  options.onAttach = function(worker) {
    ...
    if (login_tab && worker.tab==login_tab) {
      if (!logout_sent) {
        ...
        worker.port.on("logged-in", function(url, avatar, name) {
          worker.tab.close();
          connect_panel.show();
...
//        // first, make list of identities from all sites
//        var identities = {};
//
//        for (var i=0; i<sites.length; i++) {
//          var site = sites[i];
//          var data = simpleStorage.storage.accounts[site].site_data;
//
//          for (url in data.identities) { if (!data.identities.hasOwnProperty(url)) continue;
//            identities[url] = data.identities[url];
//          }
//        }
//
...
//              width: 400,
//              height: 400,
//              contentURL: Self.data.url("intro.html"),
//              contentScriptFile: [Self.data.url("lib/jquery.js"), Self.data.url("intro.js")],
...
//            });
//
//            panel.port.on("submit", function(url, name, avatar, site_identifiers) {
//              panel.hide();
//
//              if (!site_identifiers.length) {
//                require("notifications").notify({
//                  title: "Warning",
//                  text: "Aborting - no sites activated."
//                });
//                return;
//              }
//
...
//                  "method": "get"
//                },
//                "sites": site_identifiers,
//                "poll_interval": 5*60*1000
//              };
//
//              // setup polling
//              create_poll_job(simpleStorage.storage.connections.length-1);
//
//              // success message
//              require("notifications").notify({
//                title: "Added feed."
...
//                require("notifications").notify({
//                  title: "Warning",
//                  text: "Discarding introduction - no sites activated."
//                });
//                return;
//              }
//
...
function intro(target, site, url, avatar, name) {
  // make request
  Request.Request({
    url: target,
    content: {
    ...
    onComplete: function(response) {
      ...
      simpleStorage.storage.connections[id] = {
        "feed": response.json.teardownwalls_config.feed,
        "target": response.json.teardownwalls_config.target,
        "accounts": [{"site":site, "accounts":url}],
        "poll_interval": 5*60*1000
      };
      simpleStorage.storage.accounts[site].configuration[url].connections.push(id);
...
function atom(target, site, url) {
  ...
  simpleStorage.storage.connections[id] = {
    ...
    "accounts": [{site: site, account: url}],
    "poll_interval": 5*60*1000
  };
  simpleStorage.storage.accounts[site].configuration[url].connections.push(id);
...
connect_panel.port.on("account-selected", function(type, target, site, url, avatar, name) {
  if (!url) {
    require("notifications").notify({
      title: "Warning",
  ...
  if (!configuration[url]) configuration[url] = {connections: []};
  configuration[url].name = name;
  configuration[url].avatar = avatar;

  var account_data = simpleStorage.storage.accounts[site].account_data;
  if (!account_data[url]) account_data[url] = {};

  if (type=="intro") {
    intro(target, site, url, avatar, name);
  }
  else if (type=="atom") {
    atom(target, site, url);
  }
});
...
connect_panel.port.on("request-login", function(site) {
  ...
  var win = windows.open({
    ...
    onOpen: function(win) {
      var tab = win.tabs[0];
      login_tab = tab;
      logged_in_callback = function(url, avatar, name) {
  ...
  onClick: function() {
    ...
    tab.attach({
      ...
      "onMessage": function(list) {
        ...
        connect_panel.port.emit("query-account", type, url, site_list);
        ...
        for (var i=0; i<sites.length; i++) {
          ...
          var worker = pageWorker.Page({
            ...
            contentScriptFile: map_urls(config.files, "sites/"+site_identifier)
          });
