...
function get_items(feeds, toplevel_items, sub_items, start_date, items_handler) {
  ...
  var sub_statement = connection.createStatement("SELECT * FROM (SELECT * FROM items WHERE feed=:feed AND in_reply_to=:in_reply_to AND NOT verb=:like_verb ORDER BY date DESC LIMIT :limit) UNION SELECT * FROM (SELECT * FROM items WHERE feed=:feed AND in_reply_to=:in_reply_to AND verb=:like_verb) ORDER BY date DESC");
  ...
  toplevel_statement.executeAsync({
    handleResult: function(aResultSet) {
      ...
      while (row = aResultSet.getNextRow()) {
        ...
        row_params.bindByName("like_verb", "http://activitystrea.ms/schema/1.0/like");
