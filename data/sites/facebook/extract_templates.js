...
function get_post_template_new(handler) {
  ...
  */
//jQuery(".timestamp").removeClass("timestamp"); // test for alternative method
//jQuery("#home_stream > *:not(:has(.passiveName)) .livetimestamp:lt(10)").removeClass("livetimestamp");
  ...
  jQuery.each(filtered_posts, function() {
    ...
    var $comment_field_avatar = $comment_field.closest(":has("+avatar_selector+")");
    var $other_images = $avatar.add($comment_avatar);
    if ($comment_field_avatar.is($other_images)) {
      self.port.emit("log", "failed to get comment image", 0);
